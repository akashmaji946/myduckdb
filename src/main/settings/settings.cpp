...
//===--------------------------------------------------------------------===//
// Am Us Join Threshold
//===--------------------------------------------------------------------===//
void AmUsJoinThreshold::SetLocal(ClientContext &context, const Value &input) {
	auto &config = ClientConfig::GetConfig(context);
	config.am_us_join_threshold = input.GetValue<idx_t>();
}

void AmUsJoinThreshold::ResetLocal(ClientContext &context) {
	ClientConfig::GetConfig(context).am_us_join_threshold = ClientConfig().am_us_join_threshold;
}

Value AmUsJoinThreshold::GetSetting(const ClientContext &context) {
	auto &config = ClientConfig::GetConfig(context);
	return Value::UBIGINT(config.am_us_join_threshold);
}
